...
int SpotifyArduino::getCurrentlyPlaying(processCurrentlyPlaying currentlyPlayingCallback, const char *market)
{
    ...
    if (statusCode == 200)
    {
        ...
        if (!error)
        {
            ...
            current.id = item["id"].as<const char *>();
...
int SpotifyArduino::getTrackAnalysis(processAnalysis analysisCallback, const char *trackUri)
{
    char command[50] = SPOTIFY_AUDIO_ANALYSIS_ENDPOINT;
    if (trackUri[0] != 0)
    {
        char trackBuff[15];
        sprintf(trackBuff, "/%s", trackUri);
        strcat(command, trackBuff);
    }
    ...
    // Get from https://arduinojson.org/v6/assistant/
    const size_t bufferSize = analysisBufferSize;
    ...
    if (statusCode == 200)
    {
        ...
        // Apply Json Filter: https://arduinojson.org/v6/example/filter/
        StaticJsonDocument<288> filter;
        filter["track"]["tempo"] = true;
        filter["track"]["time_signature"] = true;
        filter["bars"][0]["start"] = true;
        filter["beats"][0]["start"] = true;
        filter["tatums"][0]["start"] = true;
        ...
        if (!error)
        {
            ...
            analysis.tempo = doc["track"]["tempo"].as<int>();
            analysis.signature = doc["track"]["time_signature"].as<int>();

            int numBars = doc["bars"].size();
            if (numBars > SPOTIFY_MAX_BARS)
            {
                numBars = SPOTIFY_MAX_BARS;
            }
            analysis.numBars = numBars;

            for (int i = 0; i < numBars; i++)
            {
                analysis.bars[i] = static_cast<int>((doc["bars"][i]["start"].as<float>())*1000.0);
            }

            int numBeats = doc["beats"].size();
            if (numBeats > SPOTIFY_MAX_BEATS)
            {
                numBeats = SPOTIFY_MAX_BEATS;
            }
            analysis.numBeats = numBeats;

            for (int i = 0; i < numBeats; i++)
            {
                analysis.beats[i] = static_cast<int>((doc["beats"][i]["start"].as<float>())*1000.0);
            }

            int numTatums = doc["tatums"].size();
            if (numTatums > SPOTIFY_MAX_BARS)
            {
                numTatums = SPOTIFY_MAX_BARS;
            }
            analysis.numTatums = numTatums;

            for (int i = 0; i < numTatums; i++)
            {
                analysis.tatums[i] = static_cast<int>((doc["tatums"][i]["start"].as<float>())*1000.0);
            }
