...
#define SPOTIFY_AUDIO_ANALYSIS_ENDPOINT "/v1/audio-analysis"
...
#define SPOTIFY_MAX_BARS 150
#define SPOTIFY_MAX_BEATS 600
#define SPOTIFY_MAX_TATUMS 1000
...
struct Analysis
{
  int tempo;
  int signature;
  long bars[SPOTIFY_MAX_BARS];
  int numBars;
  long beats[SPOTIFY_MAX_BEATS];
  int numBeats;
  long tatums[SPOTIFY_MAX_TATUMS];
  int numTatums;
};
...
typedef void (*processAnalysis)(Analysis analysis);
...
public:
  ...
  int getTrackAnalysis(processAnalysis analysisCallback, const char *trackUri);
